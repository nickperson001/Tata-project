  ...
  async function load() {
    ...
    try {
      const [ret, prod] = await Promise.all([
        stockApi.get<ReturnTransaction[]>('/api/stock/returns?type=sales_return', token),
        stockApi.get<{ products: Product[] }>('/api/stock/products?limit=500', token),
      ]);
      setList(ret || []);
      setProducts(prod.products || prod || []);
    } catch (e) { toast.error(e instanceof Error ? e.message : '[StockReturn] Load gagal'); }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <input className="input" type="number" min="0" value={form.priceSell} onChange={(e) => setForm({ ...form, priceSell: e.target.value })} />
            </div>
          </div>
          <div className="form-group">
