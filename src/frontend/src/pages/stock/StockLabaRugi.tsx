...
function TrendBar({ color }: { color: string }) {
  return (
    <div style={{ marginTop: '0.5rem', height: 4, borderRadius: 2, background: color, opacity: 0.3, flexShrink: 0 }} />
  );
}
    ...
    <div className="data-enter" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="grid grid-4" style={{ gap: '1rem' }}>
        ...
        <div className="card card-p">
          ...
          <TrendBar color={isProfit ? 'var(--primary)' : 'var(--danger)'} />
        </div>
      </div>
