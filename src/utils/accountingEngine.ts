...
class AccountingEngine {
  async insertJournalViaClient(
    ...
    const { entryDate = new Date(), referenceType, referenceId, description = '', channel = 'Offline', lines = [] } = opts;
  ...
  async getCoA(userId: string): Promise<CoAResult> {
    ...
    try {
      const { data, error } = await supabase
        .from('chart_of_accounts').select('*').eq('user_id', userId)
  ...
  async getLabaRugi(userId: string, startDate: string, endDate: string): Promise<LabaRugiResult> {
    ...
    try {
      const { data: entries, error: entriesError } = await supabase
        .from('journal_entries').select('id').eq('user_id', userId)
        .gte('entry_date', startDate).lte('entry_date', endDate);
      if (entriesError) throw entriesError;
      const entryIds = ((entries as any[]) || []).map(e => e.id);
      if (entryIds.length) {
        const { data: lines, error: linesError } = await supabase
          .from('journal_lines').select('account_code, debit, credit').in('entry_id', entryIds);
        ...
        if (accountCodes.length) {
          const { data: coaList } = await supabase
            .from('chart_of_accounts').select('code, type, name, normal_balance')
            .in('code', accountCodes).eq('user_id', userId);
          ((coaList as any[]) || []).forEach((a: any) => { coaMap[a.code] = a; });
        }
      ...
      const { data: trans, error: transError } = await supabase
        .from('transactions').select('type, amount, reference_type, quantity, price_buy')
      ...
      const { data: salesWithStock } = await supabase
        .from('transactions').select('quantity, price_buy')
        .eq('user_id', userId)
        .not('product_id', 'is', null)
        .not('quantity', 'is', null)
        .gte('created_at', startDate).lte('created_at', endDate) as any;
      ...
      if (totalCOGS > 0) {
        rows.push({ account_code: '5101', account_name: 'Harga Pokok Penjualan', account_type: 'cogs', total: totalCOGS });
      }
  ...
  async getBalanceSheet(userId: string, endDate?: string): Promise<BalanceSheetResult> {
    ...
    try {
      let { data: accounts, error } = await supabase
        .from('chart_of_accounts').select('code, name, type, normal_balance, balance')
        .eq('user_id', userId).eq('is_active', true);
      if (error) throw error;

      // If endDate provided, compute balance from journal entries up to that date
      if (endDate && (accounts as any[])?.length) {
        const codes = (accounts as any[]).map((a: any) => a.code);
        ...
        if (entries?.length) {
          ...
          (lines || []).forEach((l: any) => {
            ...
            balanceMap[l.account_code] = (balanceMap[l.account_code] || 0)
          ...
          accounts = (accounts as any[]).map((a: any) => {
            ...
            const bal = normalBalance === 'credit' ? -raw : raw;
      ...
      const dateStr = endDate
        ? new Date(endDate).toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' } as any)
  ...
  async getTrialBalance(userId: string): Promise<TrialBalanceResult> {
    ...
    try {
      const { data: accounts, error } = await supabase
        .from('chart_of_accounts').select('code, name, type, normal_balance, balance')
        .eq('user_id', userId).eq('is_active', true).order('code');
      if (error) throw error;
