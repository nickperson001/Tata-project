...
async function handleOnboardingStep(msg: Message, sender: string, user: any, body: string, client: any): Promise<boolean> {
  ...
  if (OB_KW_BATAL.some(k => body === k)) {
    onboardingStates.set(sender, { step: 0, timestamp: Date.now() });
    await safeReply(msg,
      `🔄 *Oke Bos, kita ulang dari awal ya!*\n\n` +
      `👇 Balas angka *1* untuk mulai simulasi.`
    );
    return true;
  }
  ...
  if (state.step === 3) {
    ...
    if (buyMatch) {
      ...
      const desc = words.filter((w: string) => parseCurrency(w) === null && !/^(beli|belanja|bayar)$/i.test(w)).join(' ') || 'gula';
      await safeReply(msg,
      ...
      setTimeout(async () => {
        try { await graduateOnboarding(msg, sender, user, client); }
        catch (_: any) { addLog('error', '[ONBOARD] graduateOnboarding failed: ' + _.message); }
      }, 1500);
    } else {
      await safeReply(msg,
        `🤔 Hampir benar Bos!\n\n` +
