...
import {
  ...
  pendingSaleDialogs,
  pendingClassificationDialogs,
  pendingTxConfirmations,
...
async function handleSaleCommand(
  msg: any,
  sender: string,
  user: any,
  productQuery: string,
  ...
  matches.forEach((p: any, i: number) => {
    text += `${i + 1}. *${p.name}* (Sisa: ${stockManager.formatQty(p.stock_current, p.unit)} ${p.unit})\n`;
  });
...
async function processSaleExecution(
  msg: any,
  sender: string,
  user: any,
  product: any,
...
async function handleTransaction(
  ...
  if (!type && amount) {
    ...
    if (ambiguousWords.length > 0) {
      pendingClassificationDialogs.set(sender, {
        amount,
        rawBody,
        body,
        ambiguousWord: ambiguousWords[0],
        descWords,
        timestamp: Date.now(),
      });
      await safeReply(
  ...
  if (matchedProduct) {
    ...
    pendingTxConfirmations.set(sender, {
      type,
      amount,
      description: finalDesc,
  ...
  pendingProductSelections.set(sender, {
    type,
    amount,
    description: finalDesc,
  ...
  return await withSenderLock(sender, async () => {
    ...
    try {
      ...
      if (pendingTxConfirmations.has(sender) && !shouldBypassDialogs(body)) {
        ...
        if (Date.now() - txConf.timestamp > 5 * 60 * 1000) {
          pendingTxConfirmations.delete(sender);
        } else {
          if (KW_BATAL.some((k: string) => body === k)) {
            pendingTxConfirmations.delete(sender);
            await safeReply(msg, '✅ Transaksi dibatalkan.');
            return;
          }
      ...
      if (pendingSaleDialogs.has(sender) && !shouldBypassDialogs(body)) {
        ...
        if (Date.now() - dialog.timestamp > 5 * 60 * 1000) {
          pendingSaleDialogs.delete(sender);
        } else {
          if (KW_BATAL.some((k: string) => body === k)) {
            pendingSaleDialogs.delete(sender);
            await safeReply(msg, '✅ Proses penjualan dibatalkan.');
            return;
          }
          ...
          if (!isNaN(choiceIndex) && choiceIndex >= 0 && choiceIndex < dialog.products.length) {
            ...
            await safeReply(
              msg,
              `⚠️ Pilihan tidak valid. Silakan balas dengan *angka 1-${dialog.products.length}* atau ketik *Batal*.`,
            );
            return;
          }
        }
      ...
      if (pendingClassificationDialogs.has(sender) && !shouldBypassDialogs(body)) {
        ...
        if (Date.now() - cDialog.timestamp > 5 * 60 * 1000) {
          pendingClassificationDialogs.delete(sender);
        } else {
          if (KW_BATAL.some((k: string) => body === k)) {
            pendingClassificationDialogs.delete(sender);
            await safeReply(msg, '✅ Konfirmasi dibatalkan.');
            return;
          }
      ...
      if (KW_DASHBOARD.some((k: string) => body === k || body.includes(k)))
      ...
      if (KW_UPGRADE.some((k: string) => body === k) || body === 'paket') {
      ...
      const saleMatch = rawBody.match(
        /^(?:jual|laku|terjual|sold)\s+(?:(tokped|tiktok(?:\s*shop)?|lazada|shopee)\s+)?(.+?)\s+(\d+(?:[.,]\d+)?)\s*(pcs|kg|gram|liter|buah|bungkus|pack|box|dus|karton|sak|meter|cm|mm)?$/i,
      );
      if (saleMatch) {
