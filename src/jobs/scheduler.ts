...
async function autoCleanCache(): Promise<void> {
  ...
  const sessionBaseDir =
    process.env.WA_SESSION_DIR || (hasPersistentStorage ? '/data' : path.join(__dirname, '../../.wwebjs_auth'));
...
async function sendUpgradeNotification(
  ...
  try {
    ...
    logError(`[ERROR] sendUpgradeNotification [${userId}]: ${err.message}`);
    return false;
  }
}
...
function initSchedulers(client: any, addLogFn?: (level: string, msg: string) => void): void {
      ...
      executeWithLock(
        ...
        async () => {
          ...
          try {
            ...
            let ok = 0;
            for (const u of users) {
              if (await sendDailyRecap(client, u.id, u.store_name, u.store_slug)) ok++;
              await sleep(300);
            }
  ...
  cron.schedule('* * * * *', () => {
    executeWithLock(
      'upgrade-notify',
      async () => {
        await checkAndNotifyUpgrades(client);
      },
      2,
    );
  });
  ...
  cron.schedule(
    ...
    () => {
      executeWithLock(
        'overdue-piutang',
        async () => {
          await checkOverduePiutang(client);
        },
        30,
      );
    },
  ...
  cron.schedule(
    ...
    () => {
      executeWithLock(
        'demo-limit',
        async () => {
          await checkDemoLimitWarning(client);
        },
        30,
      );
    },
  ...
  cron.schedule(
    ...
    () => {
      executeWithLock(
        'expiry-warning',
        async () => {
          await checkExpiryWarning(client);
        },
        30,
      );
    },
  ...
  cron.schedule(
    ...
    () => {
      executeWithLock(
        'overdue-hutang',
        async () => {
          await checkOverdueHutang(client);
        },
        30,
      );
    },
