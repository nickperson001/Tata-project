...
const logWarn = (...args: any[]) => { if (_addLog) _addLog('warn', args.join(' ')); else console.warn(...args); };
const logError = (...args: any[]) => { if (_addLog) _addLog('error', args.join(' ')); else console.error(...args); };
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          if (trashFolders.includes(entry.name)) {
            ...
            } catch (e: any) { logWarn(`[CACHE] Failed to delete ${entry.name}: ${e.message}`); }
          } else { cleanDir(fullPath, depth + 1); }
        }
      }
    } catch (err: any) { logWarn(`[CACHE] Error reading ${dirPath}: ${err.message}`); }
  }
...
async function acquireLock(jobName: string, durationMinutes = 5): Promise<boolean> {
  ...
  if (activeLocks.has(lockKey)) { logInfo(`[LOCK] ${jobName} already locked in memory`); return false; }
  try {
    const expiresAt = new Date(Date.now() + durationMinutes * 60 * 1000);
    const { data, error } = await supabase
      .from('scheduler_locks').insert([{
        job_name: jobName, locked_at: new Date().toISOString(),
        locked_by: INSTANCE_ID, expires_at: expiresAt.toISOString(),
      }]).select().single() as any;
    if (error) {
      if (error.code === '23505') {
        const { data: stale } = await supabase
          .from('scheduler_locks').select('*').eq('job_name', jobName).single() as any;
        if (stale && new Date(stale.expires_at) < new Date()) {
          ...
          const { data: retryData, error: retryErr } = await supabase
            .from('scheduler_locks').insert([{
              job_name: jobName, locked_at: new Date().toISOString(),
              locked_by: INSTANCE_ID, expires_at: new Date(Date.now() + durationMinutes * 60 * 1000).toISOString(),
            }]).select().single() as any;
          if (retryErr) { logInfo(`[LOCK] ${jobName} retry failed: ${retryErr.message}`); return false; }
          if (retryData && retryData.locked_by === INSTANCE_ID) { activeLocks.add(lockKey); logInfo(`[LOCK] ✅ ${jobName} acquired after reclaim`); return true; }
        }
      }
      logInfo(`[LOCK] ${jobName} locked by another instance`);
      return false;
    }
    if (data && data.locked_by === INSTANCE_ID) { activeLocks.add(lockKey); logInfo(`[LOCK] ✅ ${jobName} acquired by ${INSTANCE_ID}`); return true; }
    return false;
  } catch (err: any) { logError(`[LOCK] Error acquiring ${jobName}:`, err.message); return false; }
}
...
async function releaseLock(jobName: string): Promise<void> {
  ...
  } catch (err: any) { logError(`[LOCK] Error releasing ${jobName}:`, err.message); }
}
...
async function executeWithLock(jobName: string, fn: () => Promise<void>, durationMinutes = 5): Promise<void> {
  ...
  if (!acquired) return;
  try { await fn(); } catch (err: any) { logError(`[JOB] ${jobName} error:`, err.message); }
  finally { await releaseLock(jobName); }
}
...
async function sendReport(client: any, userId: string, storeName: string, periodStr: string, timeFilterIso: string): Promise<boolean> {
  try {
    ...
    return true;
  } catch (err: any) { logError(`[ERROR] sendReport [${userId}]: ${err.message}`); return false; }
}
...
async function sendUpgradeNotification(client: any, userId: string, storeName: string, status: string, expiresAt?: string): Promise<boolean> {
  try {
    ...
    if (status === 'unlimited') {
      ...
      const exp = expiresAt ? new Date(expiresAt).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' } as any) : '—';
...
async function checkAndNotifyUpgrades(client: any): Promise<void> {
  try {
    const { data: users, error } = await supabase
      ...
      .in('status', ['pro', 'unlimited']).eq('upgrade_notified', false) as any;
    if (error) { logError(`[ERROR] checkAndNotifyUpgrades: ${error.message}`); return; }
  ...
  } catch (err: any) { logError(`[ERROR] checkAndNotifyUpgrades: ${err.message}`); }
}
...
async function checkExpiredSubscriptions(client: any): Promise<void> {
  try {
    ...
    const { data: expired, error } = await supabase
      .from('users').select('id, store_name').eq('status', 'pro').lt('subscription_expires_at', now) as any;
    if (error) { logError(`[ERROR] checkExpired: ${error.message}`); return; }
    ...
    for (const u of expired) {
      ...
      try { await client.sendMessage(u.id, `⚠️ *Langganan PRO Habis - ${u.store_name}*\n\nAkun Anda kembali ke mode *DEMO* (5 transaksi/hari).\n\nKetik *Paket* untuk perpanjang langganan. 🙏`); } catch { /* ignore */ }
      await sleep(500);
    }
  } catch (err: any) { logError(`[ERROR] checkExpiredSubscriptions: ${err.message}`); }
}
...
async function broadcastMessage(client: any, message: string, target = 'all'): Promise<{ sent: number; failed: number; total: number; skipped?: boolean }> {
  ...
  } catch (err: any) { logError(`[ERROR] broadcastMessage: ${err.message}`); return { sent: 0, failed: 0, total: 0 }; }
}
...
async function processBroadcastPending(client: any): Promise<void> {
  try {
    ...
    let req: any;
    try { req = JSON.parse(data.value); } catch { return; }
    ...
    if (result.skipped) { logInfo(`[BROADCAST] Skipped duplicate`); return; }
    logInfo(`[BROADCAST] Hasil: ${JSON.stringify(result)}`);
    await supabase.from('settings').upsert({ key: 'broadcast_last_result', value: JSON.stringify({ ...result, at: new Date().toISOString() }) });
  } catch (err: any) { logError(`[ERROR] processBroadcastPending: ${err.message}`); }
}
  ...
  try {
    ...
    for (const u of users) {
      try {
        ...
        sent++;
      } catch { logError('[CRON] morning greeting failed for ' + u.id); }
      await sleep(600);
    }
    logInfo(`[CRON] Sapaan Pagi selesai: ${sent}/${users.length} terkirim.`);
  } catch (err: any) { logError(`[CRON] Sapaan Pagi error: ${err.message}`); }
}
...
async function checkStockAlerts(client: any): Promise<void> {
  ...
  try {
    ...
    const { data: allUsers } = await supabase
      .from('users').select('id, store_name, status, dashboard_token, store_slug').in('id', userIds) as any;
    ...
    for (const [userId, alerts] of Object.entries(byUser)) {
      try {
        ...
        } else { msg += `Ketik *Dashboard* di WA untuk akses portal stok.`; }
        await client.sendMessage(userId, msg);
        sent++;
      } catch (e: any) { logError(`[STOCK] Alert send error ${userId}:`, e.message); }
      await sleep(800);
    }
    logInfo(`[CRON] Stock alerts sent: ${sent}`);
  } catch (err: any) { logError(`[CRON] Stock alert error: ${err.message}`); }
}
...
async function checkOverduePiutang(client: any): Promise<void> {
  ...
  try {
    ...
    const { data: allUsers } = await supabase
      .from('users').select('id, store_name').in('id', Object.keys(byUser)) as any;
    const userMap: Record<string, any> = {};
    (allUsers || []).forEach((u: any) => { userMap[u.id] = u; });
    ...
    for (const [userId, userDebts] of Object.entries(byUser)) {
      ...
      try {
        ...
        sent++;
      } catch (e: any) { logError(`[CRON] Overdue notify error ${userId}: ${e.message}`); }
      await sleep(500);
    }
    logInfo(`[CRON] Piutang overdue notified: ${sent}/${Object.keys(byUser).length}`);
  } catch (err: any) { logError(`[CRON] checkOverduePiutang error: ${err.message}`); }
}
...
async function checkDemoLimitWarning(client: any): Promise<void> {
  ...
  try {
    ...
    const { data: users } = await supabase
      .from('users').select('id, store_name').eq('status', 'demo').eq('onboarding_status', 'active_user') as any;
    if (!users || users.length === 0) return;
    ...
    for (const u of users) {
      ...
      const { count } = await supabase
        ...
        .eq('user_id', u.id).gte('created_at', today.toISOString()) as any;
      ...
      if (count != null && count >= 3) {
        ...
        } catch (e: any) { logError(`[CRON] Demo limit notify error ${u.id}: ${e.message}`); }
      }
      await sleep(300);
    }
  } catch (err: any) { logError(`[CRON] checkDemoLimitWarning error: ${err.message}`); }
}
  ...
  try {
    ...
    for (const [userId, userHutangs] of Object.entries(byUser)) {
      ...
      try {
        const user = userMap[userId];
        if (!user) continue;
        const total = userHutangs.reduce((sum: number, h: any) => sum + Number(h.nominal_hutang) - Number(h.jumlah_dibayar || 0), 0);
        ...
        sent++;
      } catch (e: any) { logError(`[CRON] Hutang overdue notify error ${userId}: ${e.message}`); }
      await sleep(500);
    }
    logInfo(`[CRON] Hutang overdue notified: ${sent}/${Object.keys(byUser).length}`);
  } catch (err: any) { logError(`[CRON] checkOverdueHutang error: ${err.message}`); }
}
...
async function checkExpiryWarning(client: any): Promise<void> {
  ...
  try {
    ...
    for (const days of [7, 3, 1]) {
      ...
      const { data: users } = await supabase
        .from('users').select('id, store_name, subscription_expires_at')
        .eq('status', 'pro')
      ...
      for (const u of users) {
        ...
        } catch (e: any) { logError(`[CRON] Expiry notify error ${u.id}: ${e.message}`); }
        await sleep(300);
      }
    }
  } catch (err: any) { logError(`[CRON] checkExpiryWarning error: ${err.message}`); }
}
...
async function cleanupOldData(): Promise<void> {
  ...
  } catch (err: any) { logError(`[CRON] Cleanup error: ${err.message}`); }
}
...
async function sendDailyRecap(client: any, userId: string, storeName: string, storeSlug?: string): Promise<boolean> {
  try {
    ...
    const { data: trans } = await supabase.from('transactions')
      ...
      .eq('user_id', userId).gte('created_at', todayISO) as any;
    if (!trans || trans.length === 0) return false;
    ...
    const { data: cashierSales } = await supabase.from('transactions')
      .select('price_buy, quantity').eq('user_id', userId).eq('reference_type', 'cashier').gte('created_at', todayISO) as any;
    (cashierSales || []).forEach((t: any) => { hpp += (Number(t.quantity) || 0) * (Number(t.price_buy) || 0); });
    const laba = omzet - hpp - pengeluaran;
    const { data: movements } = await supabase.from('stock_movements')
      .select('quantity, products(name)').eq('user_id', userId).eq('type', 'out').gte('created_at', todayISO).order('created_at', { ascending: false }).limit(50) as any;
    ...
    const channelLines = Object.entries(channelTotals).sort((a, b) => b[1] - a[1]).map(([ch, total]) => `  • ${ch}: ${formatRupiah(total)}`).join('\n') || '  • Belum ada penjualan hari ini';
    const productLines = topProducts.length > 0 ? topProducts.map(([name, qty]) => `  ${name} — ${qty} pcs`).join('\n') : '  Belum ada penjualan produk hari ini';
    const appUrl = (process.env.APP_URL || 'https://nickridwan-tata-business-suite.hf.space').replace(/\/+$/, '');
...
function initSchedulers(client: any, addLogFn?: (level: string, msg: string) => void): void {
  ...
  cron.schedule('59 23 * * *', () => {
    executeWithLock('daily-recap', async () => {
      logInfo('[CRON] Daily Recap (23:59)...');
      try {
        const { data: users } = await supabase.from('users').select('id, store_name, store_slug')
          .eq('onboarding_status', 'active_user').in('status', ['demo', 'pro', 'unlimited']) as any;
        if (!users) return;
        let ok = 0;
  ...
  cron.schedule('0 3 * * *', () => {
    executeWithLock('cleanup', async () => {
      ...
      try { if (typeof autoCleanCache === 'function') await autoCleanCache(); } catch (cacheErr: any) { logError(`[CRON] Cache cleanup error: ${cacheErr.message}`); }
      try { await (supabase.rpc('cleanup_old_behaviors') as any); } catch (_: any) { logError('[CRON] cleanup_old_behaviors RPC failed:', _.message); }
    }, 10);
  }, tz);
  ...
  const selfPing = async () => {
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        const req = (http as any).get(`${appUrl}/ping`, { timeout: 10_000 }, (res: any) => { logInfo(`[PING] Self-ping OK → ${res.statusCode}`); resolve(); });
        req.on('error', reject);
        req.on('timeout', () => { req.destroy(); reject(new Error('timeout')); });
      });
    } catch (err: any) { logWarn(`[PING] Self-ping gagal: ${err.message}`); }
  };

  cron.schedule('*/20 * * * *', () => { selfPing(); }, tz);
